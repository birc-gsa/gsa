...
def check_make_dir(dirname: str) -> None:
    if os.path.isfile(dirname):
        error.error(f"File '{dirname}' exists and isn't a directory")
...
def relink(src: str, dst: str) -> None:
    ...
    if os.path.isfile(dst) or os.path.isdir(dst):
        error.error(f"File/directory {dst} is in the way of a symbolic link.")
    os.symlink(src, dst)
...
class test_config:
    ...
    def __init__(self, config: dict[str, typing.Any]) -> None:
        ...
        if not self.tools:
            error.error("No tools specification in configuration file")

        genomes = config['genomes'] if 'genomes' in config else None
        if not genomes:
            error.error("No genomes specification in configuration file")

        reads = config['reads'] if 'reads' in config else None
        if not reads:
            error.error("No reads specification in configuration file")
        ...
        if 'preprocess' in tool:
            for k, n in config.genomes:
                ...
                if not os.path.isfile(fastafile):
                    error.error(f"Genome file {fastafile} not found")
    ...
    for name, tool in config.tools.items():
        for (k, n), (num, length, e) in config.genomes_reads:
            ...
            if not os.path.isfile(fastqname):
                error.error(f"Couldn't find fast1 file {fastqname}")
